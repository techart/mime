 ...
 *
 * Модуль содержит небольшую базу данных наиболее распространенных MIME-типов и реализует объектный интерфейс к ней.
 * Описание типа в базе содержит следующую информацию:
 * - имя, например, text/html
 * - список расширений файлов, ассоциированных с типом
 * - признак необходимости кодирования в BASE64 (бинарные данные).
 * Модуль также реализует набор сервисных методов, позволяющий определять MIME-тип по имени файла или его расширению,
 * кодировать и декодировать строки с помощью base64 и quoted-printable, генерировать boundary.
 *
 * Опции модуля
 * - default_charset кодовая страница по умолчанию, используется при кодировании/декодировании строк и заголовков.
...
class MIME
{
    ...
    static public function type($type)
    {
        return isset(self::$type_objects[$type]) ? self::$type_objects[$type] :
            self::$type_objects[$type] = new \Techart\MIME\Type(
                $type,
                explode(
                    ',',
                    self::$types_definition[$type][0]
                ),
                self::$types_definition[$type][1] ? \Techart\MIME::ENCODING_B64 : null);
    }
     ...
     *
     * @param \Techart\IO\FS\File|string $file
     *
     * @return \Techart\MIME\Type
     */
    static public function type_for_file($file)
    {
        return self::type_for_suffix(\Techart\IO\FS::Path($file instanceof \Techart\IO\FS\File ? $file->path : (string)$file)->extension);
    }
     ...
     *
     * @return string
     * @throws \Techart\MIME\UnsupportedEncodingException
     */
    static public function encode($text, $encoding = \Techart\MIME::ENCODING_B64)
    {
        switch ($encoding) {
            ...
            case \Techart\MIME::ENCODING_8BIT:
                return $text;
            default:
                throw new \Techart\MIME\UnsupportedEncodingException($encoding);
        }
    }
     ...
     * @return string
     */
    static public function encode_qp($text, $length = \Techart\MIME::LINE_LENGTH)
    {
     ...
     * @return string
     */
    static public function split($value, $length = \Techart\MIME::LINE_LENGTH)
    {
        return wordwrap($value, $length, \Techart\MIME::LINE_END . ' ', true);
    }
     ...
     */
    static public function decode($text, $encoding = \Techart\MIME::ENCODING_B64)
    {
        switch ($encoding) {
            case \Techart\MIME::ENCODING_B64:
